...
{
    // 1. Check if Settings is open BEFORE we update it.
    bool wasSettingsOpen = Settings_IsOpen();

    // 2. ALWAYS update settings so its fade-out animation can finish, 
    //    and so it can intercept its own Escape key presses.
    Settings_Update(dt);

    // 3. If Settings WAS open at the start of this frame, we MUST return immediately.
    //    This prevents the Pause Menu from seeing the Escape key press that just closed the Settings.
    if (wasSettingsOpen) {
        return;
    }
...
void PauseMenu_Draw(AEGfxVertexList* squareMesh)
{
    ...
    // --- NEW: Draw Settings ON TOP of the pause menu ---
    // Just pass the mesh since Settings handles its own textures/fonts now!
